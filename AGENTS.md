...
- New-Client creation requires a reviewed canonical Project name and creates that Project before invoking the provider-neutral Client/Project folder boundary. Provider failure must preserve the saved records, report a reconnect/configuration or retry outcome, and reuse stable record and folder identities; editing an existing Client never implicitly creates another Project.
...
- Compare Quotes is extraction-first: automatically use the appropriate canonical active Estimate revision as baseline, then upload/select competitor evidence once, reuse the manufacturer-document analysis boundary and ask staff to review exceptions rather than transcribe proposals. Manual fields are correction evidence, not the primary creation workflow.
...
- Compare Quotes presents the normal result as one evidence-qualified report per canonical Estimate Position plus a transparent project-level summary and top-three recommendations. The baseline defines the requested reference specification but receives no incumbent, tenant or brand preference; missing evidence remains not supplied/not confirmed, detailed row mapping stays an advanced exception tool, and recommendation/report logic must be deterministic outside the UI for later document projection.
...
- Client Portal is a separately presented and separately authorised customer application over the canonical QuoteSuite backend. Disclosure requires both a tenant-enabled portal capability and an explicit Client/Project resource release. Internal staff preview is clearly identified, uses the same safe projection and never impersonates or bypasses a customer session.
- Portal authentication proves an external contact identity; resource authorization separately proves the exact tenant, Client, Project and released resource grant on every request. Unreleased Estimates and Documents remain unavailable even when their content would otherwise be customer-safe.
- Issued Estimate revisions, customer review submissions and customer decisions are immutable evidence. Amendment requests create evidence against the issued revision and require a new explicitly linked editable revision; they never edit the offer that was issued.
- Portal Review, Decline and Intent to Proceed commands require authorization, audit evidence, CSRF protection and idempotency. Intent to Proceed is not an Order or supplier commitment; canonical Order creation remains a later governed acceptance boundary.
- Customer amendment evidence is staff-reviewed before any supplier revision request. Request Supplier Revision and Raise Order to Factory are distinct internal commands; factory ordering remains fail-closed until Position acceptance, signature and internal approval are complete.
