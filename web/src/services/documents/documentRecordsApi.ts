...
export type DocumentSyncState = {state:DocumentSyncStatus;strategy:string;lastAttemptAt:string|null;lastSuccessAt:string|null;error:string|null;details?:{filesDiscovered?:number;folderMetadata?:{status:string;changed:number;rebound:number;error:string|null}}|null;cached:true};
