...
export const communicationsApi = {
  ...
  enquiryIntake:(messageId:string)=>apiFetch(`/api/communications/messages/${encodeURIComponent(messageId)}/enquiry-intake`) as Promise<{existing:null|{enquiryId:string;enquiryRef:string};communicationMessageId:string;providerMessageId:string;displayName:string;email:string;projectName:string;brief:string;attachments:Array<{id:string;fileName:string;mediaType:string;sizeBytes:number}>}>,
  createEnquiry:(messageId:string,input:unknown)=>apiFetch(`/api/communications/messages/${encodeURIComponent(messageId)}/enquiry-intake`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(input)}) as Promise<{enquiryId:string;enquiryRef:string;selectedAttachmentCount:number;storageStatus:string}>,
};
