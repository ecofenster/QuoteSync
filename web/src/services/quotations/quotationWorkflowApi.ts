...
export type IssuedQuotationLifecycleStatus="issued"|"superseded"|"withdrawn";
export type IssuedQuotationView={id:string;status:"prepared_not_sent"|"issued"|"failed";lifecycleStatus:IssuedQuotationLifecycleStatus|null;lifecycle:{status:IssuedQuotationLifecycleStatus;reason:string|null;relatedIssuedQuotationId:string|null;actorId:string;occurredAt:string}|null;clientId:string;estimateId:string;estimateRevision:number;quotationRevision:number;recipient:string;subject:string;provider:string|null;providerMessageId:string|null;communicationMessageId:string|null;preparedAt:string;issuedAt:string|null;failedAt:string|null;failureReason:string|null;commercialSnapshot:{subtotalExVatGbp:string;vatRatePercent:string;vatGbp:string;totalIncVatGbp:string};termsSnapshot:string|null;document:{id:string;fileName:string;mediaType:string;sizeBytes:number;sha256:string;downloadUrl:string}|null;communication:CommunicationMessageView|null;followUp:{id:string;dueDate:string;status:string}|null};
...
export const quotationWorkflowApi={
  ...
  downloadPreview:async(projection:CustomerQuotationProjection,estimateId:string)=>{const response=await fetch(apiUrl("/api/quotation-workflow/preview-document"),{method:"POST",headers:json,body:JSON.stringify({projection,estimateId})});if(!response.ok)throw new Error(extractApiErrorMessage(response.status,await response.text()));const blob=await response.blob(),url=URL.createObjectURL(blob),anchor=document.createElement("a"),fileName=`${projection.estimateReference}-Estimate.pdf`;anchor.href=url;anchor.download=fileName;document.body.append(anchor);anchor.click();anchor.remove();window.setTimeout(()=>URL.revokeObjectURL(url),1000);return{sizeBytes:blob.size,fileName};},
};
