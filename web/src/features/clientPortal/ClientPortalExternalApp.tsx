...
import PortalServicePanel from "./PortalServicePanel";
...
type RevisionRequest = { id:string;estimateReleaseId:string;status:string;createdAt:string;updatedEstimateIssued:boolean;completedAt:string|null };
type Portal = {
...
export default function ClientPortalExternalApp() {
  ...
  return <main className="portal-external">
    ...
    {portal.revisionRequests?.length?<section className="portal-external__command ui-card"><span className="ui-eyebrow">Estimate changes</span><h2>Requested updates</h2>{portal.revisionRequests.map(item=><article key={item.id}><strong>{item.status}</strong><p>Requested {new Date(item.createdAt).toLocaleDateString("en-GB")}. Your original comments and files remain part of this request.</p></article>)}</section>:null}
    ...
    {enabled("service") ? <PortalServicePanel projectId={session.projectId} csrf={csrf} orders={portal.orders} /> : null}
