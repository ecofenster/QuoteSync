...
export type ClientPortalEstimate = {id:string;estimateRef:string;revisionNo:number;status:"current"|"superseded"|"withdrawn"|"awaiting_review"|"accepted_order"|"declined_rejected"|"expired";commercial:CustomerSafeCommercial};
...
export function buildCustomerSafePortalProjection(input:{client:Client;documents?:CanonicalDocumentRecord[];commercialByEstimateId?:Record<string,Partial<CustomerSafeCommercial>>;releasedEstimateIds?:string[];estimateLifecycleById?:Record<string,"issued"|"superseded"|"withdrawn">;orderEstimateIds?:string[];rejectedEstimateIds?:string[];releasedDocumentIds?:string[]}):ClientPortalProjection {
  ...
  const estimates=(input.client.estimates||[]).filter((estimate)=>released.has(String(estimate.id))||ordersReleased.has(String(estimate.id))||rejectedReleased.has(String(estimate.id))).map((estimate,index):ClientPortalEstimate=>({
    ...
    status:ordersReleased.has(String(estimate.id))?"accepted_order":rejectedReleased.has(String(estimate.id))?"declined_rejected":input.estimateLifecycleById?.[String(estimate.id)]==="withdrawn"?"withdrawn":input.estimateLifecycleById?.[String(estimate.id)]==="superseded"?"superseded":index===0?"current":"superseded",
    commercial:commercialFor(estimate,commercial),
  }));
  const latestEstimate=estimates.find((estimate)=>estimate.status==="current")||estimates.find((estimate)=>estimate.status==="accepted_order")||null;
