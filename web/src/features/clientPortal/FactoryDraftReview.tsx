...
type Draft = { id:string; communicationMessageId:string; status:string; recipient:string; subject:string; bodyText:string; needsPreparation:boolean; additionalFiles:SupportingFile[];availableFiles:SupportingFile[];attachments:Array<{id:string;fileName:string;downloadUrl:string}> };
...
export default function FactoryDraftReview({orderId}:{orderId:string}) {
  ...
  const load=async()=>{const result=await apiFetch(`/api/lifecycle/orders/${orderId}`) as {factoryDraft:Draft|null};setDraft(result.factoryDraft);if(result.factoryDraft){setRecipient(result.factoryDraft.recipient);setSubject(result.factoryDraft.subject);setBody(result.factoryDraft.bodyText);setSelectedFiles(result.factoryDraft.additionalFiles.map(file=>file.id));setDocumentsReviewed(false)}};
  ...
  return <fieldset className="factory-draft-review"><legend>Factory request · {draft.status==="sent"?"Sent":"Prepared, not sent"}</legend>
    ...
    <p>Saving retains the selected files but does not send an email. Sending requires the separately reviewed delivery step.</p>
  </fieldset>;
}
