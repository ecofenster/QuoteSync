...
type HistoryPage = { total: number; offset: number; items: Array<{ id: string; recordedAt: string; recordedBy: string; checks: SavedSupplierCheck[] }> };

export default function SupplierReviewHistory({ requestId }: { requestId: string }) {
  const [page, setPage] = useState<HistoryPage | null>(null);
  const [busy, setBusy] = useState(false), [error, setError] = useState("");
  async function load(offset = 0) {
    setBusy(true); setError("");
    try { setPage(await apiFetch(`/api/lifecycle/supplier-revisions/${encodeURIComponent(requestId)}/review-history?offset=${offset}`) as HistoryPage); }
  ...
  return <details onToggle={event => { if (event.currentTarget.open && !page && !busy) void load(); }}>
    <summary>View details · earlier review history</summary>
    ...
    {page ? <>
      ...
      {page.items.map(item => <details key={item.id}>
        <summary>Saved before review on {new Date(item.recordedAt).toLocaleString("en-GB")}</summary>
        {item.checks.map((check, index) => <article key={index} className="portal-operation-detail__check">
          ...
          <p>Source: {check.after_source_reference || "Not recorded"}</p>
