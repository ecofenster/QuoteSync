...
export function createSupplierQuoteRepository(db: SqliteDatabase) {
  ...
  async function listAttachments(estimateId: string, revisionId: string): Promise<SupplierQuoteAttachment[]> {
    const rows = await db.all<Row[]>("SELECT * FROM supplier_quote_attachments WHERE estimate_id=? AND revision_id=? ORDER BY created_at,rowid", estimateId, revisionId);
  ...
  async function listPositions(estimateId: string, revisionId: string): Promise<Array<Row & { supplierReferenceTokens: string[] }>> {
    const rows = await db.all<Row[]>("SELECT * FROM supplier_quote_positions WHERE estimate_id=? AND revision_id=? ORDER BY rowid", estimateId, revisionId);
    return rows.map((row) => ({ ...row, supplierReferenceTokens: parseJson<string[]>(row.supplier_reference_tokens_json, []) }));
  }
