...
export default function EstimateCollectionView(props: Props) {
  ...
  if (currentTab === "estimates") {
    return <div className="ep-section-shell"><div className="estimate-index-table-wrap"><table className="estimate-index-table"><thead><tr><th>Estimate</th><th>Client</th><th className="estimate-index-secondary-column">Project</th><th>Positions</th><th>Area</th><th>Linear Metres</th><th>Qty</th><th>Value</th><th className="estimate-index-secondary-column">Updated</th><th className="estimate-index-actions-cell"><span className="estimate-index-action-headings"><span>Email</span><span>Follow Up</span><span>Status</span><span>Copy</span><span>Remove</span><span>Open</span></span></th></tr></thead><tbody>{items.map((item) => { const totals = estimateTotals(item),itemClient=getClientForItem(item); return <tr key={item.id} data-estimate-ref={item.estimateRef}><td><strong>{item.estimateRef}</strong><small title={item.statusExplanation||undefined}>{item.displayStatus||item.status}</small>{item.deletionRestricted?<small title={item.deletionReason||undefined}>Cannot delete · archive only</small>:null}</td><td>{item.clientName || item.clientReference || "—"}</td><td className="estimate-index-secondary-column">{item.projectAddress || "—"}</td><td>{item.positions.length}</td><td>{formatMeasure(totals.totalSquareMetres)} m²</td><td>{formatMeasure(totals.totalLinearMetres)} lm</td><td>{totals.totalQty}</td><td>{formatMoney(Number(customerValues[item.id] || 0))}</td><td className="estimate-index-secondary-column">{item.updatedAt ? new Date(item.updatedAt).toLocaleDateString("en-GB") : "—"}</td><td className="estimate-index-actions-cell"><span className="estimate-index-actions"><button type="button" className="ui-button" aria-label={`Email ${item.estimateRef}`} title="Email" onClick={()=>{setSendModalEstimateId(item.id);setSendModalOpen(true)}}>✉</button><button type="button" className="ui-button" aria-label={`Follow up ${item.estimateRef}`} title="Follow Up" onClick={()=>addFollowUpForEstimateService({pickerClient:itemClient,estimateId:item.id,opts:{days:3,sendEmail:true,needsCall:true},apiFetchJson,activeUserName,alertFn:alert,logError:console.error})}>◷</button><select className="ui-input" aria-label={`Estimate Status ${item.estimateRef}`} value={item.outcome??"Open"} onChange={event=>persistEstimateOutcome(itemClient.id,item.id,event.currentTarget.value as EstimateOutcome)}><option>Open</option><option>Order</option><option>Lost</option></select><button type="button" className="ui-button" aria-label={`Copy ${item.estimateRef}`} title="Copy Estimate" onClick={()=>copyEstimateForClient(itemClient,item.id)}>⧉</button><button type="button" className="ui-button" aria-label={`${item.deletionRestricted?"Archive":"Delete"} ${item.estimateRef}`} title={item.deletionRestricted?(item.deletionReason||"Archive issued Estimate"):"Delete Estimate"} onClick={()=>confirmDeleteEstimate(item.id)}>{item.deletionRestricted?"Archive":"×"}</button><button type="button" className="ui-button ui-button--primary" aria-label={`Open ${item.estimateRef}`} title="Open Estimate" onClick={() => onToggleEstimate(item.id)}>Open</button></span></td></tr>; })}</tbody></table>{items.length === 0 ? <div className="ui-empty-state"><Small>{emptyText}</Small></div> : null}</div></div>;
  }
