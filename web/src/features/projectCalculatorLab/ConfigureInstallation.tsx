...
import InstallationTravelReview from "./InstallationTravelReview";
import {
...
export default function ConfigureInstallation({ scenario }: { scenario: CalculatorScenario }) {
  ...
  const projectType = text(profile.projectType ?? scenario.options?.projectType) || "new_build";
  ...
  const persistCompanySelection = async () => {
    ...
    try {
      ...
      const updated = await projectCalculatorLabApi.updateInstallationProfile(scenario.id, {
        ...
        productivityCrewSize: selectedTeam.normalCrewSize,
        costedCrewSize: selectedTeam.normalCrewSize,
        route,
      });
  ...
  return <>
    ...
    <InstallationTravelReview scenario={scenario}/>
    {open && createPortal(<div className="ui-modal-backdrop" role="presentation"><section ref={advancedDialog} className="ui-modal calculator-lab__installation-modal" role="dialog" aria-modal="true" aria-labelledby="installation-config-title"><header><div><h3 id="installation-config-title">Advanced Installation</h3><small>Routine postcode, company and team choices remain in Installation. Change exceptional programme assumptions here.</small></div><button type="button" className="ui-button" autoFocus onClick={() => { setOpen(false); advancedTrigger.current?.focus(); }}>Close</button></header>
      <form onSubmit={(event) => { event.preventDefault(); void saveAdvanced(event.currentTarget); }}><div className="calculator-lab__installation-groups">
        <fieldset><legend>Company / Team</legend><div className="costing-sheet__facts"><span>Company <b>{selectedCompany?.name ?? (text(savedCompanySnapshot.name) || "Not selected")}</b></span><span>Team <b>{selectedTeam?.name ?? scenario.selectedInstallationTeam?.name ?? "Not selected"}</b></span><span>Required concurrent crew <b>{requiredCrew}</b></span></div><label>Productivity crew size<input name="productivityCrewSize" type="number" min="1" step="1" className="ui-input" defaultValue={String(profile.productivityCrewSize ?? profile.crewSize ?? selectedTeam?.normalCrewSize ?? 2)} /><small>Controls installation throughput.</small></label><label>Costed installation crew size<input name="costedCrewSize" type="number" min="1" step="1" className="ui-input" defaultValue={String(profile.costedCrewSize ?? profile.crewSize ?? selectedTeam?.normalCrewSize ?? 2)} /><small>Additional people are costed for productive installation days without increasing throughput.</small></label><label>Additional attendance travel days<input name="additionalAttendanceTravelDays" type="number" min="0" step="1" className="ui-input" defaultValue={String(profile.additionalAttendanceTravelDays ?? 0)} /><small>Full paid travel days for costed people above the productivity crew.</small></label><label>Additional attendance travel expense<input name="additionalAttendanceTravelCost" type="number" min="0" step="0.01" className="ui-input" defaultValue={String(profile.additionalAttendanceTravelCost ?? "0.00")} /><small>Reviewed travel cost where the additional attendee has a different origin or travel pattern.</small></label></fieldset>
        <fieldset><legend>Travel / Vehicles</legend><label>Travel mode<select name="travelMode" className="ui-input" defaultValue={String(profile.travelMode ?? "auto")}><option value="auto">Auto / Recommended</option><option value="daily_travel">Daily Travel</option><option value="stay_away">Stay Away</option><option value="manual">Manual</option></select></label><label>Vehicles<input name="vehicleCount" type="number" min="1" className="ui-input" defaultValue={String(profile.vehicleCount ?? 1)} /></label><label>Mileage / mile<input name="mileageRate" className="ui-input" defaultValue={String(profile.mileageRate ?? "0.55")} /></label><label>Mobilisation/offload/set-out hours<input name="mobilisationSetOutHours" className="ui-input" placeholder="Use day allowance where configured" defaultValue={String(profile.mobilisationSetOutHours ?? "")} /></label><label>Delivery / offload / set-out days<input name="deliveryOffloadSetOutDays" type="number" min="0" step="0.5" className="ui-input" placeholder="Installation-type default" defaultValue={String(profile.deliveryOffloadSetOutDays ?? "")} /></label><label>Snagging days<input name="snaggingDays" type="number" min="0" step="0.5" className="ui-input" placeholder="Installation-type default" defaultValue={String(profile.snaggingDays ?? "")} /></label></fieldset>
