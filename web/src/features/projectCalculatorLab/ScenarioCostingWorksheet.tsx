  ...
  return (
    ...
    >
      ...
      <td>
        ...
        {row.estimatePositionId ? <span className="costing-sheet__position-actions"><button className="ui-button" onClick={() => positionAction("up")} aria-label={`Move ${row.displayReference} up`}>↑</button><button className="ui-button" onClick={() => positionAction("down")} aria-label={`Move ${row.displayReference} down`}>↓</button><button className="ui-button" onClick={() => positionAction("duplicate")}>Duplicate</button><button className="ui-button" aria-label={`${row.displayReference} alternative`} onClick={() => positionAction("alternative")}>Alternative</button><button className="ui-button" onClick={() => positionAction("delete")}>Delete</button></span> : null}
      </td>
    </tr>
  );
}
...
export default function ScenarioCostingWorksheet({
  ...
  return (
    <>
      {commercialView === "internal" ? <SupplierCommercialReview
        scenarioId={scenario.id}
        policies={commercialScenario.supplierCommercialPolicies ?? []}
        onSaveSupplier={onUpdateSupplierCommercial}
      /> : null}
        ...
        <div className="project-costing__layout">
          ...
          <main className="costing-sheet">
            <div className="costing-sheet__title">
              ...
              <div
                className={
                  dirty ? "costing-sheet__unsaved" : "costing-sheet__saved"
                }
              >
                {dirty
                  ? "Unsaved markup changes"
                  : `Saved · Revision ${scenario.revisionNumber}`}
              </div>
            </div>
            {error ? (
              <p
            ...
            {equipment.length ? (
              <Section
                ...
                summary={`${equipment.length} selected`}
                cost={money(String(equipmentCost))}
                converted={money(equipmentCost)}
                markup={markupDraft.equipment}
                sale={money(equipmentSale)}
                open={open === "equipment"}
                onToggle={() => toggle("equipment")}
                onMarkupChange={(value) => editMarkup("equipment", value)}
                onMarkupKeyDown={markupKeys("equipment")}
                invalid={Boolean(
                  validateMarkupPercentage(markupDraft.equipment),
                )}
              >
            ...
            {installVisible ? (
              <Section
                ...
                converted={money(installationGbp)}
                markup={markupDraft.installation}
                sale={money(installationSale)}
              ...
              <div className="costing-sheet__summary-categories">
                ...
                {[
                  ...
                  ...(!siteVisitAllocatedToProducts && Number(siteVisitCost) > 0
                    ...
                    : []),
                  ["Equipment Hire", equipmentSale],
                  ["Installation", installationSale],
