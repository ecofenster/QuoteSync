...
export function ClientProjectsPanel({ clientId }: { clientId: string }) {
  ...
  const [adding, setAdding] = useState(false), [busy, setBusy] = useState(false), [provisioningProjectId, setProvisioningProjectId] = useState<string | null>(null);
  ...
  async function create() {
    ...
    try {
      ...
      setName(""); setSiteAddress(""); setAdding(false); setSelectedProjectId(project.id);
      setMessage(project.driveProvisioning?.message || "Project saved. Folder creation status was not returned.");
      await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Project could not be created."); }
    finally { setBusy(false); }
  }

  async function provisionFiles(projectId: string) {
    setProvisioningProjectId(projectId); setMessage("");
    try { setMessage((await commercialIdentityApi.provisionProjectDrive(projectId)).message); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Client / Project folders could not be created."); }
    finally { setProvisioningProjectId(null); }
  }
  ...
  return <section className="client-projects ui-card" aria-labelledby="client-projects-title">
    ...
    <p>Creates missing folders in your connected storage. Existing folders are reused.</p>
    ...
    <div className="client-projects__list">{projects.map((project) => <article key={project.id}><div><strong>{project.name}</strong><small>{project.contextYear || "Year review"} · {project.status}</small></div><span>{project.estimateCount} Estimate{project.estimateCount === 1 ? "" : "s"} · {project.orderCount} Order{project.orderCount === 1 ? "" : "s"} <button type="button" className="ui-button ui-button--ghost" disabled={provisioningProjectId === project.id} onClick={() => void provisionFiles(project.id)}>{provisioningProjectId === project.id ? "Creating folders…" : "Create Client / Project Folders"}</button> <button type="button" className="ui-button ui-button--ghost" aria-expanded={selectedProjectId === project.id} onClick={() => setSelectedProjectId((current) => current === project.id ? null : project.id)}>Files</button></span></article>)}{!projects.length ? <p>No canonical Projects yet. Historical Client project text remains available until reviewed migration.</p> : null}</div>
