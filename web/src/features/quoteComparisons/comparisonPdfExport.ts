...
function findingStack(offer:QuoteComparisonReportOffer):PdfNode[] {
  const result:PdfNode[]=[{text:offer.isBaselineReference?"Selected supplier":offer.compliance.label,bold:true,color:offer.isBaselineReference?"#3a4a2a":"#17211d",margin:[0,0,0,2]}];
  if(offer.isBaselineReference){result.push({text:offer.customerFinding.matches||"Defines the selected reference; not independently verified.",margin:[0,0,0,3]});return result;}
  ...
  if(offer.customerFinding.confirmation)result.push({text:[{text:"Confirm: ",bold:true},offer.customerFinding.confirmation],margin:[0,0,0,3]});
  if(offer.customerFinding.disclosure)result.push({text:offer.customerFinding.disclosure,margin:[0,0,0,3]});
  result.push({text:[{text:"Recommendation: ",bold:true},offer.customerFinding.recommendation],margin:[0,0,0,3]});
  return result;
}
...
function drawingCell(offer:QuoteComparisonReportOffer,assets:Map<string,string|null>):PdfNode {
  const cells=offer.drawings.map(drawing=>{
    ...
    return {stack:[image?{image,fit:[74/Math.max(1,Math.min(2,offer.drawings.length)),58],alignment:"center",margin:[0,3,0,3]}:{text:"Image unavailable",alignment:"center",color:"#64735f",fontSize:8,margin:[0,22,0,20]},{text:`${drawing.sourceReference}${drawing.sourcePage?` · p${drawing.sourcePage}`:""}`,fontSize:7.5,color:"#53615a",alignment:"center"}],margin:[1,1,1,1]};
  });
  return {table:{widths:cells.map(()=>"*"),body:[cells]},layout:{hLineColor:()=>"#d9e0db",vLineColor:()=>"#d9e0db",paddingLeft:()=>1,paddingRight:()=>1,paddingTop:()=>1,paddingBottom:()=>1}};
}

function offerDetails(offer:QuoteComparisonReportOffer):PdfNode[] {
  const sources=(offer.attributes.systemEvidence??[]).map(source=>`${source.system}${source.systemThermal?` · Uw ${source.systemThermal.raw} for ${source.systemThermal.standardSizeMm?`${source.systemThermal.standardSizeMm.width} × ${source.systemThermal.standardSizeMm.height} mm standard window`:"standard-size window"}`:""}${source.systemSourcePage?` · source p${source.systemSourcePage}`:""}`).join("; ");
  const cells=offer.presentationFields.map(field=>({columns:[fieldMarker(field.status),{width:"*",stack:[{text:field.label,bold:true,color:"#53615a",fontSize:8},{text:field.value,fontSize:9,lineHeight:1.12}]}],columnGap:1,margin:[0,0,0,1]}));
  const body=Array.from({length:Math.ceil(cells.length/2)},(_,row)=>[cells[row*2],cells[row*2+1]??{text:""}]);
  const result:PdfNode[]=[{table:{widths:["*","*"],body},layout:{hLineWidth:()=>0,vLineWidth:()=>0,paddingLeft:()=>0,paddingRight:()=>3,paddingTop:()=>1,paddingBottom:()=>1}}];
  if(offer.annualHeatLoss.status!=="reference")result.push({text:[{text:"Annual heat loss: ",bold:true,color:"#53615a"},offer.annualHeatLoss.display],fontSize:8.5,margin:[0,2,0,1]});
  if(sources)result.push({text:[{text:"System evidence: ",bold:true,color:"#53615a"},`${sources}. Position-specific evidence takes precedence.`],fontSize:8.5,margin:[0,2,0,1]});
  return result;
}
...
function positionPage(position:QuoteComparisonReport["positions"][number],assets:Map<string,string|null>,firstPosition:boolean):PdfNode {
  const rows=position.offers.map(offer=>[
    ...
    {stack:offerDetails(offer)},
    {stack:priceDetails(offer)},
    {stack:findingStack(offer)},
  ]);
  ...
  const rowTable=(row:PdfNode[],offer:QuoteComparisonReportOffer):PdfNode=>({table:{headerRows:1,dontBreakRows:true,widths:[80,70,300,90,"*"],body:[[{text:`DRAWING · POSITION ${position.reference}`,style:"tableHeader"},{text:"SUPPLIER / ITEM",style:"tableHeader"},{text:"WHAT IS OFFERED",style:"tableHeader"},{text:"PRICE",style:"tableHeader"},{text:"FINDING",style:"tableHeader"}],row]},layout:{fillColor:(index:number)=>index===0?"#e9eeeb":offer.isBaselineReference?"#f2f6ed":offer.isAlternative?"#fff8eb":null,hLineColor:()=>"#cbd5cf",vLineColor:()=>"#d9e0db",paddingLeft:()=>4,paddingRight:()=>4,paddingTop:()=>4,paddingBottom:()=>4},fontSize:9,lineHeight:1.18});
  return {pageBreak:firstPosition?"before":undefined,stack:[
    ...rows.map((row,index)=>({unbreakable:true,stack:[...(index===0?[heading]:[]),rowTable(row,position.offers[index])]})),
...
function buildDocument(report:QuoteComparisonReport,context:PrintContext,assets:Map<string,string|null>) {
  ...
  const content:PdfNode[]=[
    ...
    ...report.positions.map((position,index)=>positionPage(position,assets,index===0)),
  ];
...
function buildPositionDocument(report:QuoteComparisonReport,context:PrintContext,position:QuoteComparisonReport["positions"][number],assets:Map<string,string|null>) {
  ...
  const compactDetails=(offer:QuoteComparisonReportOffer):PdfNode[]=>{
    const fields=new Map(offer.presentationFields.map((field)=>[field.key,field]));
    const line=(label:string,keys:string[])=>({text:[{text:`${label}: `,bold:true,color:"#53615a"},keys.map((key)=>fields.get(key)?.value).filter(Boolean).join(" · ")||"Not supplied"],margin:[0,0,0,2]});
    return [line("Product / operation",["product_material"]),line("Dimensions / quantity",["dimensions","quantity"]),line("Finishes",["internal_finish","external_finish"]),line("Glass",["glass"]),line("Security / hardware",["security_hardware"]),line("Thermal",["uw","ug","g","lt"])];
  };
