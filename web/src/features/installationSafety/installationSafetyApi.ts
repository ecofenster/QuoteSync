...
export type RamsVersion={id:string;version:number;state:"draft"|"issued"|"superseded";title:string;workSummary:string;siteConditions:{description?:string};sourceSnapshot?:{attendanceReview?:{attendanceStart:string;attendanceEnd:string;requiredQualificationTypes:string[]}};hazards:RamsHazard[];methodSteps:string[];emergencyArrangements:string;publicProtection:string;wasteArrangements:string;specialistAssessment:string;supportingDocumentIds:string[];reviewerName:string|null;reviewerRole:string|null;reviewedAt:string|null;competentPersonConfirmed:boolean;issuedAt:string|null;pdfFileName:string|null;downloadUrl:string|null};
