# =====================================================================
# QuoteSync — Phase 4C: Extract Default Clients seed data from App.tsx
# Script: 20260227_04_phase4c_extract_default_clients.ps1
...
#
# Moves:
# - DEFAULT_CUSTOMER_ADDRESS constant
# - makeDefaultClients() function (seed/demo clients)
#
# Notes:
# - To avoid coupling to App.tsx helpers, the extracted function becomes:
#     makeDefaultClients(opts)
#   where opts provides uid() and nextClientRef() used by the seed.
# - No UI/layout/logic changes (same seed content; same ID generation functions).
#
# Safety:
# - Auto-backup touched files to _backups\<timestamp>\
# - Fail fast if anchors ambiguous or target file exists
# - Prints diagnostics if block is not found
# =====================================================================
...
# Block: DEFAULT_CUSTOMER_ADDRESS + function makeDefaultClients(): Client[] { ... }
$blockPattern = '(?s)const DEFAULT_CUSTOMER_ADDRESS\s*=\s*\[[\s\S]*?\]\.join\(""\s*\\n\s*""\);\s*\r?\n\r?\nfunction makeDefaultClients\(\): Client\[\]\s*\{[\s\S]*?\r?\n\}\s*\r?\n'
...
if ($matches.Count -ne 1) {
  ...
  $c1 = Find-Context $appTxt "DEFAULT_CUSTOMER_ADDRESS" 25
  if ($c1.Count -gt 0) { $c1 | ForEach-Object { Write-Host $_ } } else { Warn "No DEFAULT_CUSTOMER_ADDRESS found." }
  Write-Host ""
  $c2 = Find-Context $appTxt "function makeDefaultClients" 25
...
# Transform makeDefaultClients signature to accept helpers from App (no behaviour change)
$blockOut = $block
$blockOut = $blockOut -replace 'function makeDefaultClients\(\): Client\[\]\s*\{', 'export function makeDefaultClients(opts: { uid: () => string; nextClientRef: (n: number) => string }): Client[] {'

# Ensure all calls match new signature
$blockOut = $blockOut -replace 'nextClientRef\(', 'opts.nextClientRef('
$blockOut = $blockOut -replace 'uid\(\)', 'opts.uid()'

# Keep Models.asClientId as-is (already in block); if not, still fine
# Export DEFAULT_CUSTOMER_ADDRESS
$blockOut = $blockOut -replace 'const DEFAULT_CUSTOMER_ADDRESS\s*=', 'export const DEFAULT_CUSTOMER_ADDRESS ='
...
# Insert import near other feature imports: after DefaultsEditor import if present, else after EstimatePickerTabs import
$inserted = $false
...
# Update call site: makeDefaultClients()
...
# Replace ONLY the first call (the initial seed), keep any future references intact by requiring exact call token
