# QuoteSync — Phase 2: Extract Estimate Picker Tabs into separate file
...
if (-not (Test-Path (Join-Path $WebRoot "package.json"))) { Fail "Could not detect web root. Run from C:\Github\QuoteSync\web\ps1_patches" }
Ok "Detected web root: $WebRoot"

$SrcDir  = Join-Path $WebRoot "src"
$AppPath = Join-Path $SrcDir "App.tsx"
if (-not (Test-Path $AppPath)) { Fail "src\App.tsx not found." }
...
# Ensure feature folder
$FeatDir = Join-Path $SrcDir "features\estimatePicker"
New-Item -ItemType Directory -Force -Path $FeatDir | Out-Null

$NewFile = Join-Path $FeatDir "EstimatePickerTabs.tsx"

# Write new component (content embedded in script)
$NewContent = @'
...
'@
Set-Content -LiteralPath $NewFile -Value $NewContent -Encoding UTF8
Ok "Wrote $NewFile"

# Patch App.tsx
$txt = Get-Content -LiteralPath $AppPath -Raw -Encoding UTF8

$importAnchor = 'import GridEditor from "./components/GridEditor";'
if (-not $txt.Contains($importAnchor)) { Fail "Import anchor not found: GridEditor import" }

$importLine = 'import EstimatePickerTabs from "./features/estimatePicker/EstimatePickerTabs";'
if (-not $txt.Contains($importLine)) {
  $txt = $txt.Replace($importAnchor, $importAnchor + "`r`n" + $importLine)
  Ok "Inserted EstimatePickerTabs import."
} else {
  Ok "EstimatePickerTabs import already present."
}

$start = $txt.IndexOf("{/* Tabs (Estimate Picker only) */}")
if ($start -lt 0) { Fail "Tabs block start marker not found." }

$endMarker = "`n                  )}`n                </div>`n              </Card"
$endPos = $txt.IndexOf($endMarker, $start)
if ($endPos -lt 0) { Fail "Tabs block end marker not found." }

$replaceEnd = $endPos + ("`n                  )}`n").Length
...
$txt = $txt.Substring(0, $start) + $replacement + $txt.Substring($replaceEnd)
Set-Content -LiteralPath $AppPath -Value $txt -Encoding UTF8
Ok "Patched App.tsx to use EstimatePickerTabs."

Write-Host ""
Ok "Phase 2 complete."
Write-Host "Backup: $BackupDir" -ForegroundColor Cyan
Write-Host "Next: run npm run dev from C:\Github\QuoteSync\web" -ForegroundColor Cyan
