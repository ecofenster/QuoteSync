# =====================================================================
# QuoteSync — Phase 4B (v2): Extract DefaultsEditor component from App.tsx
# Script: 20260227_01_phase4b_extract_defaults_editor_v2.ps1
# Run from: PS C:\Github\QuoteSync\web\ps1_patches>
#
# Prereqs (fail-fast if missing):
# - src\features\catalog\defaultCatalog.ts
# - src\features\estimateDefaults\defaultEstimateDefaults.ts  (Phase 4A)
#
# Creates:
...
#   to avoid coupling (renders identically to current inline primitives).
#
# Safety:
# - Auto-backup touched files to _backups\<timestamp>\
# - Fail fast if anchors ambiguous, block not found, or target file exists
# =====================================================================
...
if (!(Test-Path $catalogPath)) { Fail "Missing prerequisite: src\features\catalog\defaultCatalog.ts (Phase 4 catalog extraction)" }
...
# Robust block: from Defaults Editor comment to just before Main App comment
$blockPattern = '(?s)\/\*\s*========================\s*\r?\n\s*Defaults Editor[\s\S]*?\r?\n\*\/\s*\r?\n\r?\nfunction DefaultsEditor\([\s\S]*?\r?\n\}\s*\r?\n\r?\n(?=\/\*\s*========================\s*\r?\n\s*Main App)'
...
# Insert import after EstimatePickerTabs import (must exist once)
