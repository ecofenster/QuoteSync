...
Fails fast if web root not detected.
#>
...
function Add-Tree([string]$dir, [string]$prefix, [System.Collections.Generic.List[string]]$lines) {
    ...
    for ($i=0; $i -lt $children.Count; $i++) {
        ...
        if ($c.PSIsContainer) {
            ...
            Add-Tree $c.FullName $nextPrefix $lines
        } else {
            $size = $c.Length
