# QuoteSync patch (SAFEWRITE): Fix Client Notes backwards typing + Handle local file paths for "Open link"
# This patch rewrites src\features\estimatePicker\EstimatePickerTabs.tsx with a corrected version based on your uploaded file.
# - Notes: removes dangerouslySetInnerHTML control loop; uses ref + effect sync only when editor not focused.
# - Files: local paths/file:// are copied to clipboard + user instructed to paste into Explorer; web URLs open normally.
#
...
export default function EstimatePickerTabs(props: Props) {
  ...
  const openClientFileLink = (raw: string) => {
    ...
    if (isFileUrl || isWinPath) {
      ...
      alert(
        "This is a local file path (e.g. Google Drive Desktop / NAS / SharePoint sync). Browsers can't open local paths directly.\n\n" +
      ...
      {clientFiles.map((f) => (
        <div key={f.id} style={{ borderRadius: 14, border: "1px solid #e4e4e7", padding: 12, background: "#fff" }}>
          ...
          <div style={{ marginTop: 10, display: "flex", justifyContent: "flex-end" }}>
            <Button variant="secondary" onClick={() => openClientFileLink(f.url)}>
              Open link
            </Button>
          </div>
        </div>
