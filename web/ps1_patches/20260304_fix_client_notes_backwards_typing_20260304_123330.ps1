# QuoteSync patch: Fix Client Notes typing backwards (caret reset) in EstimatePickerTabs.tsx
# Cause: contentEditable editor re-rendered with dangerouslySetInnerHTML on each keystroke, forcing caret to start -> text appears reversed.
# Fix: remove dangerouslySetInnerHTML from the editor div, and sync innerHTML only when NOT focused using a ref + useEffect.
#
# IMPORTANT:
...
#   Unblock-File .\20260304_fix_client_notes_backwards_typing_20260304_123330.ps1
#   pwsh -ExecutionPolicy Bypass -File .\20260304_fix_client_notes_backwards_typing_20260304_123330.ps1
...
# SAFEWRITE full file content (no placeholders)
$txt = @'
...
export default function EstimatePickerTabs(props: Props) {
  ...
  // Client Notes editor:
  // Do NOT use dangerouslySetInnerHTML during typing (it resets caret and causes "backwards" text).
  const clientNotesRef = useRef<HTMLDivElement | null>(null);
  const clientNotesHasFocusRef = useRef(false);

  useEffect(() => {
    const el = clientNotesRef.current;
    if (!el) return;

    // If user is currently editing, don't touch DOM (preserves caret position).
    if (clientNotesHasFocusRef.current) return;

    const desired = (clientNoteDraftHtml ?? "") as string;
    if ((el.innerHTML ?? "") !== desired) el.innerHTML = desired;
  }, [clientNoteDraftHtml, pickerClient?.id]);
...
{estimatePickerTab === "client_notes" && (
  <div style={{ display: "grid", gap: 10 }}>
    ...
    <div
      ...
      ref={clientNotesRef}
      onFocus={() => {
        clientNotesHasFocusRef.current = true;
      }}
      onBlur={() => {
        clientNotesHasFocusRef.current = false;
      }}
      style={{
        minHeight: 120,
        borderRadius: 14,
...
Write-Host ""
Write-Host "DONE. Refresh the browser; dev server does not need restarting." -ForegroundColor Cyan
