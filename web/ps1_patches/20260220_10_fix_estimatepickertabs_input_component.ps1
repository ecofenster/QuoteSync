...
# Script: 20260220_10_fix_estimatepickertabs_input_component.ps1
...
# Preconditions
Ensure-Once "EstimatePickerTabs.tsx uses <Input ...>" $txt "<Input\b"
Ensure-NotPresent "EstimatePickerTabs.tsx already defines Input" $txt "function\s+Input\s*\("
Ensure-NotPresent "EstimatePickerTabs.tsx already defines Input const" $txt "const\s+Input\s*="

# Insert local Input primitive between Button and Pill
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  ...
  // Keep styling minimal + consistent with existing UI primitives in this file.
