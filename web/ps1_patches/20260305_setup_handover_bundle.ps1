<# 
QuoteSync Handover System — Setup + Snapshot Generator
Creates/updates:
...
Rules:
  ...
  - End by running: npm run dev (per standing rule)
#>
...
Set-Content -Path (Join-Path $handoverDir "SNAPSHOT.txt") -Value ($snap -join "`r`n") -Encoding UTF8
Ok "Wrote: _handover\SNAPSHOT.txt"

Ok "Handover system setup complete."
Write-Host ""
Write-Host "Next: upload _handover\HANDOVER.md + _handover\SNAPSHOT.txt + _handover\CONTEXT.json in any new chat." -ForegroundColor Cyan
Write-Host ""

# Per standing rule: run dev server at end
Set-Location $webRoot
Write-Host "Starting dev server (npm run dev) from: $webRoot" -ForegroundColor Cyan
npm run dev
