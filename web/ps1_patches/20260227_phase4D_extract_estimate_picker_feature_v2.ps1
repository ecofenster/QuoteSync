# QuoteSync Phase 4D (v2) - Extract Estimate Picker Feature Container
# Run from: PS C:\Github\QuoteSync\web\ps1_patches>
# Fixes vs v1:
# - PowerShell string interpolation bug ($label:) fixed via ${label}
# - Import anchor made tolerant (regex), to avoid exact-string mismatch
#
# This patch:
...
Assert-ContainsOnce $txt '// estimate picker' "App.tsx estimate picker comment anchor"
Assert-ContainsOnce $txt '\{view\s*===\s*"estimate_picker"\s*&&\s*pickerClient\s*&&\s*\(' "App.tsx estimate picker render anchor"

# 1) Ensure React import includes useRef (tolerant)
