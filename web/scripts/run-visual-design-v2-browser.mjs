...
try {
  ...
  for (const theme of themes) {
    ...
    for (const [width, height] of viewports) {
      ...
      await evaluate("(()=>{const dashboard=document.querySelector('[data-visual-lab-screen=dashboard]'),costing=document.querySelector('[data-visual-lab-screen=project-costing]'),administration=document.querySelector('[data-visual-lab-screen=administration]');dashboard.style.display='';costing.style.display='none';administration.style.display='none';dashboard.scrollIntoView({block:'start',inline:'start'})})()");
      await new Promise((resolve) => setTimeout(resolve, 80));
      await screenshot(`${theme}--main-dashboard--${width}x${height}.png`);
      await evaluate("(()=>{document.querySelector('[data-visual-lab-screen=dashboard]').style.display='none';document.querySelector('[data-visual-lab-screen=project-costing]').style.display='';document.querySelector('[data-visual-lab-screen=administration]').style.display=''})()");
      ...
      const state = await evaluate(`(()=>{
        ...
        const worksheet=document.querySelector('.project-costing__worksheet-header'),section=document.querySelector('.costing-sheet__section'),shellInnerBox=document.querySelector('.app-shell__inner').getBoundingClientRect(),lockup=document.querySelector('[data-testid=brand-lockup]');
  ...
  console.log(JSON.stringify({ themes: themes.length, approvedV2Themes: approvedV2Themes.length, viewports: viewports.length, screenshots: themes.length * viewports.length * 3 + themes.length + 5 + approvedV2Themes.length * viewports.length * 4 + approvedV2Themes.length * 2, results, interactionResults, detailResults, restored, outputDir }, null, 2));
} finally {
