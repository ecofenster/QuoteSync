...
export async function linkSupplierPositionToEstimate(db,{estimateId,sourcePositionId,sourceRevisionId,sourceQuoteId=null,quotationReference=null,sourceSequence,displayReference,quantity,widthMm,heightMm,classification='standard',alternativeTo=null,supplierName=null,supplierCode=null,product=null,productSystem=null,preferredEstimatePositionId=null,replacesSourcePositionId=null}){
  ...
  if(!matched&&!reviewed){
    const candidate={positionRef:displayReference,qty:quantity,widthMm,heightMm},matches=positions.filter(position=>{const links=Array.isArray(position.supplierEvidenceLinks)?position.supplierEvidenceLinks:[];const sameLegacyDealer=sourceQuoteId==null&&links.length>0&&links.every(link=>link.sourceQuoteId==null)&&links.some(link=>(supplierCode&&normalized(link.supplierCode)===normalized(supplierCode))||(!supplierCode&&supplierName&&normalized(link.supplierName)===normalized(supplierName)));const sameAggregate=!links.length||(sourceQuoteId!=null&&links.some(link=>link.sourceQuoteId===sourceQuoteId))||sameLegacyDealer;return sameAggregate&&signature(position)===signature(candidate)&&!links.some(link=>link.sourceRevisionId===sourceRevisionId);});
