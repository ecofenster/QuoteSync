...
export async function saveConfiguredEstimatePosition(db,{estimateId,positionId,configuredContract,projection}){
  const row=await db.get('SELECT positions_json FROM estimates WHERE id=?',estimateId);if(!row)return null;const positions=parsePositions(row.positions_json).map(normalizeCanonicalEstimatePosition),index=positions.findIndex(position=>position.id===positionId);if(index<0)return null;
  const current=positions[index];positions[index]={...current,...projection,id:current.id,sourceSequence:current.sourceSequence,classification:current.classification,alternativeTo:current.alternativeTo,supplier:current.supplier,sourceProvenance:current.sourceProvenance,supplierEvidenceLinks:current.supplierEvidenceLinks,configuredContract,origin:current.origin==='supplier_imported'?'supplier_imported':current.origin??'b92_configured',configurationState:current.supplierEvidenceLinks?.length?'imported_configured':'configured',matchStatus:current.matchStatus};
...
export async function addConfiguredEstimatePosition(db,{estimateId,position}){const row=await db.get('SELECT positions_json FROM estimates WHERE id=?',estimateId);if(!row)return null;const positions=parsePositions(row.positions_json).map(normalizeCanonicalEstimatePosition),normalized=normalizeCanonicalEstimatePosition({...position,sourceSequence:positions.length,origin:'b92_configured',configurationState:'configured'},positions.length);if(positions.some(item=>item.id===normalized.id))return positions.find(item=>item.id===normalized.id);positions.push(normalized);await db.run('UPDATE estimates SET positions_json=?,updated_at=? WHERE id=?',JSON.stringify(positions),new Date().toISOString(),estimateId);return normalized;}
