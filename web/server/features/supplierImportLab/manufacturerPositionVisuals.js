...
const scriptPath = path.resolve('server/features/supplierImportLab/renderEmfToPng.ps1');
...
function runEmfRenderer(inputPath, outputPath) {
  return new Promise((resolve, reject) => execFile('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', scriptPath, '-InputPath', inputPath, '-OutputPath', outputPath], { windowsHide: true, timeout: 60_000 }, (error, stdout) => {
...
export async function extractDocxManufacturerVisuals(buffer, attachment, { visualRoot } = {}) {
  ...
  for (const [cellOrdinal, cell] of [...documentXml.matchAll(/<w:tc\b[\s\S]*?<\/w:tc>/g)].entries()) {
    ...
    for (const relationshipId of relationshipIds) {
      ...
      const originalName = `source.${extension}`; await writeFile(path.join(directory, originalName), source); let derivative = null;
      try {
...
export function mapManufacturerVisualsToRows(rows, candidates, quotation = {}) {
  ...
  for (const candidate of candidates || []) {
    const matches = groups.get(normalizeReference(candidate.customerReference)) || [];
