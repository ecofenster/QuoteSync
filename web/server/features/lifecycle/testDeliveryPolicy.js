...
export function createTestDeliveryPolicy(environment = process.env) {
  ...
  return {
    ...
    assertRecipient(value, role) {
      if (!enabled) throw Object.assign(new Error('Test journey delivery is disabled; preview only.'), { status: 409, code: 'test_delivery_disabled' });
    ...
    publicStatus() { return { enabled, configured: enabled && Boolean(customer && factory), customerConfigured: Boolean(customer), factoryConfigured: Boolean(factory), deliveryMode: enabled ? 'test_allowlist' : 'preview_only' }; },
  };
}
