...
export function createSupplierQuotesService(db, { attachmentRoot = resolveAttachmentRoot(), extractDocument = extractSupplierDocument, parseFields = parseCommercialFields, parseSummary = parseCommercialSummary, derivePreviews = derivePdfPositionPreviews, failureInjector = async () => {}, evidenceRefreshFailureInjector = async () => {}, fileSupplierAttachments = true } = {}) {
  ...
  async function stageCanonicalDocumentForReview(input) {
    ...
    const existing=await db.get(`SELECT attachment.*,revision.supplier_quote_id,quote.supplier_code FROM supplier_quote_attachments attachment JOIN supplier_quote_revisions revision ON revision.id=attachment.revision_id AND revision.estimate_id=attachment.estimate_id JOIN supplier_quotes quote ON quote.id=revision.supplier_quote_id AND quote.estimate_id=revision.estimate_id WHERE attachment.source_canonical_document_id=? AND attachment.estimate_id=?`,canonicalDocumentId,estimateId);
    if(existing){
      if(existing.supplier_code!==supplier.supplier_code)throw Object.assign(new Error('This retained document was previously imported for a different supplier. Review its supplier relationship in Files before continuing; no imported evidence was changed.'),{status:409,code:'canonical_document_supplier_conflict'});
      if(existing.sha256!==createHash('sha256').update(bytes).digest('hex'))throw Object.assign(new Error('This provider file has changed since it was retained for Manufacturer Import. The earlier source is preserved. Review the changed file in Files and save it as a separate document revision before importing it; nothing was overwritten.'),{status:409,code:'canonical_document_content_changed'});
    ...
    const sha256=createHash('sha256').update(bytes).digest('hex'),matching=await db.get(`SELECT attachment.*,revision.supplier_quote_id FROM supplier_quote_attachments attachment JOIN supplier_quote_revisions revision ON revision.id=attachment.revision_id AND revision.estimate_id=attachment.estimate_id JOIN supplier_quotes quote ON quote.id=revision.supplier_quote_id AND quote.estimate_id=revision.estimate_id WHERE attachment.estimate_id=? AND attachment.sha256=? AND attachment.role<>'derived_artifact' AND attachment.parser_eligible=1 AND quote.supplier_code=? ORDER BY attachment.created_at,attachment.rowid LIMIT 1`,estimateId,sha256,supplier.supplier_code);
    if(matching){if(!matching.source_canonical_document_id)await db.run('UPDATE supplier_quote_attachments SET source_canonical_document_id=? WHERE id=? AND source_canonical_document_id IS NULL',canonicalDocumentId,matching.id);const attached={...matching,source_canonical_document_id:matching.source_canonical_document_id||canonicalDocumentId},documents=[{quoteId:matching.supplier_quote_id,revisionId:matching.revision_id,attachmentId:matching.id}],quote=await getQuote(estimateId,matching.supplier_quote_id),revision=await getRevision(estimateId,matching.supplier_quote_id,matching.revision_id);return{duplicate:true,matchedBy:'content_sha256',quote,revision,attachments:[mapAttachment(attached)],documents,review:await prepareImportReview(estimateId,documents)}}
  ...
  async function prepareImportReview(estimateId, documents) {
    ...
    return {
      ...
      metadata: {
        ...
        commercialScope: first.fields.commercialScope ?? null,
  ...
  async function extractAndLoadSupplierCosts(estimateId, scenarioId, documents, confirmation = {}) {
    ...
    for (const { quote, revision, selected, isComplementaryPackage } of groups.values()) {
      ...
      const identity=createSupplierImportOperationIdentity({quote,revision,scenarioId,attachments,selectedRowKeys:confirmation.selectedRowKeys||selected.flatMap(item=>item.fields.rows.map(row=>`${item.attachment.id}:${row.ordinal}`)),reviewedRows:selectedPositionRows,reviewedCurrency:revision.currency,extractionContract:{extractorVersion:EXTRACTOR_VERSION,fieldParserVersion:FIELD_PARSER_VERSION,summaryParserVersion:SUMMARY_PARSER_VERSION}});
      ...
      try {
        ...
        try {
          ...
          const expectedCostSourceIds=new Set([...costs.values()].map(({key})=>stableRevisionEvidenceId('supplier-extra',revision.id,key)));
          const attachmentPlaceholders=attachments.map(()=>'?').join(',');
          const priorCostRows=attachmentPlaceholders?await db.all(`SELECT c.id cost_row_id,c.source_extra_id FROM project_calculator_estimate_supplier_costs c WHERE c.scenario_id=? AND c.source_revision_id=? AND c.source_attachment_id IN (${attachmentPlaceholders})`,scenarioId,revision.id,...attachments.map(item=>item.id)):[];
          const summaryEvidenceAvailable=selected.some(item=>Boolean(item.summaryResult?.summary));
          let supersededCostRows=0;
          if(summaryEvidenceAvailable)for(const prior of priorCostRows)if(!expectedCostSourceIds.has(prior.source_extra_id)){
            supersededCostRows+=(await db.run('DELETE FROM project_calculator_estimate_supplier_costs WHERE id=? AND scenario_id=?',prior.cost_row_id,scenarioId)).changes;
            await db.run('UPDATE supplier_quote_extras SET superseded_at=COALESCE(superseded_at,?),superseded_by_operation_id=COALESCE(superseded_by_operation_id,?) WHERE id=?',completedAt,identity.operationId,prior.source_extra_id);
          }
          if(supersededCostRows)warnings.push(`${supersededCostRows} stale supplier commercial projection row${supersededCostRows===1?' was':'s were'} superseded by the reviewed source re-import; original extracted evidence remains retained.`);
          ...
          const priorCommercialLink=await db.get('SELECT commercial_policy_json FROM project_calculator_supplier_quote_revisions WHERE scenario_id=? AND revision_id=?',scenarioId,revision.id);let persistedCommercialPolicy=commercialPolicy;
          if(priorCommercialLink?.commercial_policy_json){let priorPolicy={};try{priorPolicy=JSON.parse(priorCommercialLink.commercial_policy_json);}catch{}persistedCommercialPolicy={...commercialPolicy,...priorPolicy,quotedCurrency:commercialPolicy.quotedCurrency,quotedAmount:commercialPolicy.quotedAmount,manufacturerListAmount:commercialPolicy.manufacturerListAmount,sourceQuotedPriceBasis:commercialPolicy.sourceQuotedPriceBasis,sourceProductCommercialEvidence:commercialPolicy.sourceProductCommercialEvidence,sourceCommercialClassification:commercialPolicy.sourceCommercialClassification};}
          await db.run(`INSERT INTO project_calculator_supplier_quote_revisions(scenario_id,supplier_quote_id,revision_id,import_run_id,commercial_policy_json,currency,linked_at) VALUES(?,?,?,?,?,?,?) ON CONFLICT(scenario_id,revision_id) DO UPDATE SET import_run_id=excluded.import_run_id,currency=excluded.currency,linked_at=excluded.linked_at,commercial_policy_json=excluded.commercial_policy_json`, scenarioId, quote.id, revision.id, runId,JSON.stringify(persistedCommercialPolicy), effectiveCurrency, completedAt);
