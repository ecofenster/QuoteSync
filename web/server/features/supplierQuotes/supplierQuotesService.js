      ...
      try {
        ...
        try {
          ...
          for (const [sourceSequence,{ row, key, attachment, sourceDocuments }] of [...products.values()].entries()) {
            ...
            const identity=[normalizeReference(row.displayReference),row.widthMm,row.heightMm,row.quantity].join('|'),existingCostingRow=existingProductQueues.get(identity)?.shift();if(existingCostingRow){const reviewed=existingCostingRow.source_review_status==='reviewed';await db.run('UPDATE project_calculator_estimate_product_rows SET source_snapshot_json=?,total_price_amount=?,currency=?,classification=?,included_in_current_estimate=?,alternative_to_reference=?,updated_at=? WHERE id=?',JSON.stringify(snapshot),row.totalPrice,row.currency,reviewed?existingCostingRow.classification:row.classification||'standard',reviewed?existingCostingRow.included_in_current_estimate:row.includedInSupplierTotal===false?0:1,reviewed?existingCostingRow.alternative_to_reference:row.alternativeTo||null,completedAt,existingCostingRow.id);continue;}
