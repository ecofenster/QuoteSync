...
export function createGmailProvider(googleWorkspace, { pageSize = 30 } = {}) {
  ...
  async function findFactoryReceipt(messageId){
    ...
    return json(await googleWorkspace.googleFetch(`${API}/messages/${encodeURIComponent(matches.messages[0].id)}?format=full`));
  }
  return { labels, list, readMessage, readThread, currentHistoryId, listHistory, watch, stopWatch, command, send, createDraft, attachment,findFactoryReceipt };
}
