...
export function createPortalSecurityService(db, options = {}) {
  ...
  async function buildProjectPortalProjection({ session, clientId, projectId, internalPreview = false }) {
    ...
    const revisionRows = enabled.has("review_estimate") ? await db.all(`SELECT sr.id,sr.source_release_id,sr.workflow_state,sr.created_at,sr.completed_at
      FROM supplier_revision_requests sr JOIN estimate_revision_releases rel ON rel.id=sr.source_release_id
      WHERE rel.tenant_id=? AND rel.client_id=? AND rel.project_id=? AND sr.status<>'cancelled' ORDER BY sr.created_at DESC`,tenantId,clientId,projectId).catch(()=>[]) : [];
