...
export function calculateInstallationProgramme({ positions = [], rules = {}, profile = {}, selectedTeam = null }) {
  ...
  const routeInput=profile.route;
  const suppliedRouteNumber=value=>value!==null&&value!==undefined&&value!==''&&Number.isFinite(Number(value))&&Number(value)>=0;
  const outwardRouteConfirmed=!!routeInput&&(!!routeInput.snapshotId||routeInput.manuallyOverridden===true&&typeof routeInput.overrideReason==='string'&&!!routeInput.overrideReason.trim())&&suppliedRouteNumber(routeInput.oneWayDurationMinutes)&&suppliedRouteNumber(routeInput.oneWayMiles);
  if(!outwardRouteConfirmed)reviewRequired.push('Travel time not confirmed. Review the installer base, site address and retained route or a manual override with its basis. Travel costing is incomplete.');
