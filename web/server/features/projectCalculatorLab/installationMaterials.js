...
export function calculateInstallationMaterials({ positions, rules, options, catalogue }) {
  ...
  // Retain the exact selected variant alongside quantity evidence for downstream operational documents.
  for(const [key,selected] of [['brackets',selectedBracket],['frameScrews',selectedFrameScrew],['substrateFixings',selectedSubstrate]]){
    purchasing[key].productId=selected?.id??null;
    purchasing[key].productLabel=selected?.label??null;
  }
