...
export function createInstallationWorkforceService(db) {
  ...
  return {
    ...
    async saveQualification(input) {
      ...
      const existing=await db.get('SELECT * FROM installation_installer_qualifications WHERE id=?',qualificationId);
      const evidenceId=input.evidenceDocumentId??existing?.evidence_document_id;
      let verificationStatus=['recorded_unverified','verified','not_supplied'].includes(input.verificationStatus)?input.verificationStatus:'recorded_unverified';
      let evidence=null;if(evidenceId){evidence=await db.get('SELECT * FROM canonical_documents WHERE id=? AND removed_at IS NULL AND trashed=0',String(evidenceId));if(!evidence)throw Object.assign(new Error('The selected evidence file is unavailable. Upload it through Files, then select that retained document.'),{code:'invalid_installation_workforce'});}
      if(evidence?.installer_id&&evidence.installer_id!==installerId)throw Object.assign(new Error('The evidence belongs to another installer. Select this installer\'s retained qualification document.'),{code:'invalid_installation_workforce'});
      ...
      if(existing&&(existing.installer_id!==installerId||existing.qualification_type_code!==typeCode||(renewalOfId&&existing.supersedes_qualification_id!==renewalOfId)))throw Object.assign(new Error('This qualification belongs to another installer, type or renewal. Reopen the installer and review the retained record.'),{code:'invalid_installation_workforce'});
