...
export function sourceBackedGlassFallback(position,row){
  ...
  const glazingLabel=field?.label==='Glazing'||field?.label==='Glazing required'&&field.section==='Glazing';
  if(!row.sourceRowId||!field||!glazingLabel||field.rawValue!==glazing?.value||!Number.isInteger(field.sourcePage)||field.sourcePage<1||!panes)return null;
