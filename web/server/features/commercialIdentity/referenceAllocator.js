...
export function formatCanonicalReference(kind, value, year = null, prefix = null) {
  ...
  if (kind === "service") {
    const servicePrefix = String(prefix || "").trim().toUpperCase();
    if (!/^[A-Z0-9]+(?:-[A-Z0-9]+)*-SER$/.test(servicePrefix)) throw Object.assign(new Error("A configured service reference prefix ending in -SER is required."), { status: 422, code: "service_reference_prefix_required" });
    return `${servicePrefix}-${pad3(value)}`;
  }
...
export async function allocateCanonicalReference(db, { kind, year = null, prefix = null, entityId, reason = "canonical_allocation", reconciliationPlanId = null, now = new Date().toISOString() }) {
  ...
  for (let attempts = 0; attempts < 10000; attempts += 1) {
    ...
    const reference = formatCanonicalReference(kind, row.last_value, year, prefix);
