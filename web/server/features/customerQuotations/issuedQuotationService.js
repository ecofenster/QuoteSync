...
export function createIssuedQuotationService(db, options = {}) {
  ...
  async function prepare(input) {
    ...
    if (revisionRequest) {
      ...
      if (!revisionRequest.returned_document_id || !revisionRequest.verified_at || unresolved || missing || await staleSupplierReviewCount(db,revisionRequest)) throw Object.assign(new Error("Resolve every requested and unrelated material supplier-revision change against the current returned document before issuing the successor Estimate."), { status: 409, code: "supplier_revision_verification_required" });
    }
    ...
    const supplierReviewSnapshot=await supplierReviewPreparationSnapshot(db,revisionRequest);
    const termsSnapshot=JSON.stringify(reviewedTerms),idempotencyKey = hash(JSON.stringify({ estimateId, estimateRevision, quotationRevision, projection, recipient, termsSnapshot, ...(supplierReviewSnapshot?{supplierReviewSnapshot}:{}) }));
    ...
    try{
      ...
      await db.run(`INSERT INTO issued_quotations(id,idempotency_key,client_id,estimate_id,estimate_revision,quotation_revision,document_id,status,recipient,subject,provider,provider_message_id,communication_message_id,prepared_at,issued_at,failed_at,failure_reason,commercial_snapshot_json,terms_snapshot,created_at,updated_at,supplier_review_snapshot) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, issuedQuotationId, idempotencyKey, clientId, estimateId, estimateRevision, quotationRevision, document.id, "prepared_not_sent", recipient, subject, null, null, communication.id, timestamp, null, null, null, JSON.stringify(commercialSnapshot), termsSnapshot, timestamp, timestamp,supplierReviewSnapshot);
    }catch(cause){
  ...
  async function finalize(row, communication) {
    ...
    try {
      ...
      await db.run("UPDATE supplier_revision_requests SET workflow_state='revised_customer_estimate_issued',completed_at=COALESCE(completed_at,?),updated_at=? WHERE successor_estimate_id=? AND verified_at IS NOT NULL AND status<>'cancelled'",issuedAt,issuedAt,row.estimate_id);
  ...
  async function send(id, overrides = {}) {
    ...
    const currentRequest=await db.get("SELECT * FROM supplier_revision_requests WHERE successor_estimate_id=? ORDER BY created_at DESC LIMIT 1",row.estimate_id);
    if(currentRequest){
