...
import { addConfiguredEstimatePosition, mergeSourceOwnedEstimatePositions, replaceEditableEstimatePositions, resolveCanonicalAlternativeRelationships, saveConfiguredEstimatePosition } from '../features/estimatePositions/canonicalEstimatePositions.js';
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await assertEstimateRevisionEditable(db, req.params.id);
    ...
    await db.run(
      ...
      [
        ...
        normalizeJsonValue(defaults_json, {}),
        JSON.stringify(mergeSourceOwnedEstimatePositions(
          (() => { try { return JSON.parse(current.positions_json || '[]'); } catch { return []; } })(),
          (() => {
            if (positions_json == null) {
              try { return JSON.parse(current.positions_json || '[]'); } catch { return []; }
            }
            if (Array.isArray(positions_json)) return positions_json;
            try { const parsed = JSON.parse(positions_json); return Array.isArray(parsed) ? parsed : []; } catch { return []; }
          })()
        )),
        normalizeJsonValue(order_meta_json, {}),
        outcome ?? 'Open',
