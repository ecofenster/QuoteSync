...
import {createOrderInstallationPlanStore} from '../features/installationSafety/orderInstallationPlanStore.js';
...
export function createInstallationDocumentsRouter({databasePromise,environment=process.env,attachmentRoot}={}){
  ...
  const planStore=async db=>createOrderInstallationPlanStore({databasePath:(await db.all('PRAGMA database_list')).find(item=>item.name==='main')?.file});
  router.get('/estimates/:estimateId',async(req,res)=>{try{
    ...
    const plans=await db.all('SELECT id,order_id orderId,version,review_reason reviewReason,reviewed_at reviewedAt FROM order_installation_plans WHERE estimate_id=? AND client_id=? ORDER BY reviewed_at DESC,id DESC LIMIT 20',estimate.id,estimate.client_id);
    if(req.query.orderPlanId!==undefined){
      const id=req.query.orderPlanId;if(typeof id!=='string'||!id||id.length>200||typeof req.query.orderId!=='string')throw problem('Reopen the selected Order plan.',400);
      const selected=await db.get('SELECT id,order_id orderId,version,review_reason reviewReason,reviewed_at reviewedAt FROM order_installation_plans WHERE id=? AND order_id=? AND estimate_id=? AND client_id=?',id,req.query.orderId,estimate.id,estimate.client_id);
      if(!selected)throw problem('The selected reviewed plan is unavailable for this Order. No replacement was selected.',404);
      if(!plans.some(item=>item.id===id)){plans.unshift(selected);if(plans.length>20)plans.pop();}
    }
    res.json({capability:'installation-document-preparation-v1',orderPlanCapability:'reviewed-order-installation-v1',access:'local_development_only',clientId:estimate.client_id,estimateReference:estimate.estimate_ref,revision:estimate.revision_no,scenarios,orders,plans,documents,total:count.total,offset,limit:10});
  ...
  for(const [suffix,method] of [['preview','preview'],['save','save']])router.post(`/estimates/:estimateId/order-plans/${suffix}`,async(req,res)=>{try{const {db,estimate}=await context(req.params.estimateId);if(req.body?.clientId!==estimate.client_id)throw problem('The selected Client does not own this Estimate.',403);res.json(await(await planStore(db))[method]({...req.body,estimateId:estimate.id},{clientId:estimate.client_id,actorId:CURRENT_APP_USER.id}));}catch(error){fail(res,error)}});
  router.get('/estimates/:estimateId/order-plans',async(req,res)=>{try{const {db,estimate}=await context(req.params.estimateId);const orderId=req.query.orderId;if(typeof orderId!=='string'||!orderId||orderId.length>200)throw problem('Select the exact Order for plan history.',400);const order=await db.get('SELECT id FROM orders WHERE id=? AND client_id=? AND source_estimate_id=?',orderId,estimate.client_id,estimate.id);if(!order)throw problem('Order not found for this Estimate.',404);res.json(await(await planStore(db)).list({clientId:estimate.client_id,estimateId:estimate.id,orderId,offset:Number(req.query.offset||0)}));}catch(error){fail(res,error)}});
  router.get('/estimates/:estimateId/order-plans/:planId',async(req,res)=>{try{const {db,estimate}=await context(req.params.estimateId);res.json(await(await planStore(db)).read(req.params.planId,{clientId:estimate.client_id,estimateId:estimate.id,orderId:req.query.orderId}));}catch(error){fail(res,error)}});
