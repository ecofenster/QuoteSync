...
export function createLifecycleRouter({ databasePromise, serviceOptions } = {}) {
  ...
  router.post('/supplier-enquiries/:supplierEnquiryId/check-followup-delivery',async(req,res)=>{try{res.json(await(await service()).reconcileSupplierDelivery(req.params.supplierEnquiryId,CURRENT_APP_USER.id,true));}catch(error){fail(res,error)}});
