...
export function createCommunicationsRouter({ databasePromise = dbPromise, serviceOptions } = {}) {
  ...
  router.get("/messages", async (req, res) => { try { res.json(await (await service()).listMailbox({ folder: String(req.query.folder || "inbox"), query: String(req.query.q || ""), pageToken: req.query.page_token ? String(req.query.page_token) : null, mode: req.query.mode === "conversation" ? "conversation" : "message" })); } catch (error) { fail(res, error); } });
  router.post("/sync", async (req, res) => { try { res.json(await (await service()).syncMailbox({ folder: String(req.body?.folder || "inbox"), query: String(req.body?.query || ""), pageToken: req.body?.pageToken ? String(req.body.pageToken) : null, mode: req.body?.mode === "conversation" ? "conversation" : "message" })); } catch (error) { fail(res, error); } });
  ...
  router.delete("/messages/:providerMessageId/links", async (req, res) => { try { res.json(await (await service()).unlinkRelationship(req.params.providerMessageId, req.body || {})); } catch (error) { fail(res, error); } });
  router.get("/change-state", async (_req, res) => { try { res.json(await (await service()).changeState()); } catch (error) { fail(res, error); } });
  router.post("/notifications/google-workspace", async (req, res) => { try { const result = await (await service()).receiveNotification(req.body || {}, { headers: req.headers, ip: req.ip }); res.status(result.outcome === "processed" ? 202 : 200).json(result); } catch (error) { fail(res, error); } });
