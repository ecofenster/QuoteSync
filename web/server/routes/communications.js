...
export function createCommunicationsRouter({ databasePromise = dbPromise, serviceOptions } = {}) {
  ...
  router.post("/sync", async (req, res) => { try { res.json(await (await service()).syncMailbox({ folder: String(req.body?.folder || "inbox"), query: String(req.body?.query || ""), pageToken: req.body?.pageToken ? String(req.body.pageToken) : null })); } catch (error) { fail(res, error); } });
