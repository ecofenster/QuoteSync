...
export function createQuotationWorkflowRouter({ databasePromise, serviceOptions } = {}) {
  ...
  router.get("/estimates/:estimateId/customer-terms",async(req,res)=>{try{res.json(await(await service()).customerTerms(req.params.estimateId))}catch(error){fail(res,error)}});
  router.put("/estimates/:estimateId/customer-terms",async(req,res)=>{try{res.json(await(await service()).saveCustomerTerms(req.params.estimateId,{...(req.body||{}),reviewedBy:CURRENT_APP_USER.id}))}catch(error){fail(res,error)}});
  router.post("/preview-document", async (req, res) => { try { const projection = req.body?.projection; await(await service()).assertCustomerTerms(req.body?.estimateId,projection);const bytes = await renderCustomerLifecyclePdf({ kind: "estimate", projection, context: { reference: projection?.estimateReference, revision: projection?.commercialRevision, documentDate: projection?.previewDate } }); res.setHeader("Content-Type", "application/pdf"); res.setHeader("Content-Disposition", `attachment; filename=\"${String(projection?.estimateReference || 'Estimate').replace(/[^A-Za-z0-9_-]+/g, '-')}-Estimate.pdf\"`); res.setHeader("Content-Length", String(bytes.length)); return res.send(bytes); } catch (error) { fail(res, error); } });
  ...
  router.post("/issued/:id/withdraw", async (req, res) => { try { res.json(await (await service()).withdraw(req.params.id, { ...(req.body || {}), actorId: CURRENT_APP_USER.id })); } catch (error) { fail(res, error); } });
