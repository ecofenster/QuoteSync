...
export default function App() {
  ...
  const [addClientIsBusiness, setAddClientIsBusiness] = useState(false);
...
const [draftEmail, setDraftEmail] = useState("");
  ...
  function createClient(type: ClientType) {
    const newClient: Client = {
      ...
      contactPerson: type === "Business" ? draftClientName.trim() : "",
      estimates: [],
    };
              ...
              <Card style={{ minHeight: 520 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                  <div>
                    ...
                    <Small>Open a client to view details and work with estimates.</Small>
                  </div>

                  <Button variant="primary" onClick={() => { setAddClientIsBusiness(false); setShowAddClient(true); }}>
                    Add Client
                  </Button>
                </div>
                  ...
                  <div style={{ marginTop: 14, borderRadius: 16, border: "1px solid #e4e4e7", padding: 12, background: "#fff" }}>
                    <div style={{ display: "grid", gap: 10 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <H3>Add client</H3>
                        <Small>Type: {addClientIsBusiness ? "Business" : "Individual"}</Small>
                      </div>

                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8 }}>
                        <input type="checkbox" checked={addClientIsBusiness} onChange={(e) => setAddClientIsBusiness(e.currentTarget.checked)} />
                        <Small>Business customer</Small>
                      </div>
    ...
    <div>
      ...
      <Input value={draftClientName} onChange={setDraftClientName} placeholder="Name" />
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                        ...
                        <Button variant="primary" onClick={() => createClient(addClientIsBusiness ? "Business" : "Individual")}>
                          Create Client
                        </Button>
                      </div>
                    </div>
            ...
            {view === "client_details" && selectedClient && (
              <Card style={{ minHeight: 520 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <Button variant="secondary" onClick={() => { setSelectedClientId(null); setView("customers"); }}>
                      Back
                    </Button>
                    <Button variant="primary" onClick={() => createEstimateForClient(selectedClient)}>
                      New Estimate
                    </Button>
                  </div>
                </div>
                ...
                <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div style={{ borderRadius: 14, border: "1px solid #e4e4e7", padding: 12, background: "#fff" }}>
                    ...
                    <div style={{ marginTop: 10, display: "grid", gap: 8, fontSize: 13 }}>
                      {selectedClient.type === "Business" && (
                        <div><strong>Business:</strong> {selectedClient.businessName || "—"}</div>
                      )}
                      {selectedClient.type === "Business" && (
                        <div><strong>Contact:</strong> {selectedClient.contactPerson || "—"}</div>
                      )}
                      <div><strong>Email:</strong> {selectedClient.email || "—"}</div>
                      <div><strong>Mobile:</strong> {selectedClient.mobile || "—"}</div>
                      <div><strong>Home:</strong> {selectedClient.home || "—"}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ borderRadius: 14, border: "1px solid #e4e4e7", padding: 12, background: "#fff" }}>
                    <H3>Project</H3>
                    <div style={{ marginTop: 10, display: "grid", gap: 8, fontSize: 13 }}>
                      <div><strong>Name:</strong> {selectedClient.projectName || "—"}</div>
                      <div><strong>Address:</strong></div>
                      <div style={{ whiteSpace: "pre-wrap", borderRadius: 12, border: "1px solid #e4e4e7", padding: 10, background: "#fafafa" }}>
                        {selectedClient.projectAddress || "—"}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ marginTop: 14 }}>
                  <H3>Estimates</H3>
                  <Small>Open an estimate to continue working.</Small>

                  <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
                    {selectedClient.estimates.length === 0 && (
                      <div style={{ fontSize: 13, color: "#71717a" }}>No estimates yet.</div>
                    )}
                    ...
                    {selectedClient.estimates.map((e) => (
                      ...
                      >
                        ...
                        <div style={{ display: "flex", gap: 10 }}>
                          <Button variant="primary" onClick={() => openEstimateDefaults(selectedClient.id, e.id)}>
                            Open
                          </Button>
                          <Button variant="secondary" onClick={() => openEstimateWorkspace(selectedClient.id, e.id)}>
                            Workspace
                          </Button>
                        </div>
                      </div>
                          ...
                          <div style={{ display: "grid", gap: 12 }}>
                            ...
                            <div style={{ borderRadius: 14, border: "1px solid #e4e4e7", padding: 12 }}>
                              ...
                              <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
                                ...
                                <Small>When unticked, you can override the same defaults below.</Small>
                              </div>
                            </div>
