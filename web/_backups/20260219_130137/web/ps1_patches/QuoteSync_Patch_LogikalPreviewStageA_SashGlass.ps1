# QuoteSync Patch — LogiKal Preview Stage A refinement (sash + glass + hex field label)
# Run from: PS C:\Github\QuoteSync\web\ps1_patches>
# This patch updates: src\components\GridEditor.tsx
...
Write-Host "Run directory: $runFrom" -ForegroundColor Cyan

# Ensure we're in the expected patches folder (best-effort)
$expected = "C:\Github\QuoteSync\web\ps1_patches"
if ($runFrom -ne $expected) {
  Write-Host "Note: Expected run directory: $expected" -ForegroundColor Yellow
}
...
Ok "Set-Location $webRoot"

$target = Join-Path $webRoot "src\components\GridEditor.tsx"
...
$backup = Join-Path $backupDir ("GridEditor.tsx.stageA_sashglass." + $ts + ".bak")
Copy-Item $target $backup -Force
Ok "Backup created: $backup"

# Write updated file (UTF8)
$payload = @'
...
Write-Host "1) If dev server is running, the page should hot-reload. Otherwise restart npm run dev." -ForegroundColor Cyan
Write-Host "2) Hard refresh the browser (Ctrl+F5)." -ForegroundColor Cyan
