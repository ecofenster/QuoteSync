...
test("customer changes produce a carried-forward working revision, attached change summary and evidence-gated successor issue",async t=>{
  ...
  await assert.rejects(()=>issuance.prepare({estimateId:successor.id,clientId:'client-a',estimateRevision:2,quotationRevision:1,projection,recipient:'a@example.test'}),error=>error.code==='supplier_revision_verification_required');
  const correctedInput={...recheck(verified.checks.find(row=>row.field_key==='external_finish')),requestedChange:'Corrected staff transcription',beforeValue:'Recorded original white',expectedValue:'Grey',beforeSourceReference:'Corrected original source reference'};
  const corrected=await lifecycle.verifySupplierRevision(request.id,{reviewedBy:'staff-1',checks:[correctedInput]});
  const retainedCorrection=corrected.checks.find(row=>row.field_key==='external_finish');
  assert.equal(retainedCorrection.requested_change,correctedInput.requestedChange);assert.equal(retainedCorrection.before_value,correctedInput.beforeValue);assert.equal(retainedCorrection.expected_value,'Grey');assert.equal(retainedCorrection.before_source_reference,correctedInput.beforeSourceReference);assert.equal(retainedCorrection.status,'needs_review');assert.equal(corrected.issueAllowed,false);
  const correctionHistory=await source.db.all('SELECT checks_json FROM supplier_revision_review_history WHERE request_id=?',request.id);assert.ok(correctionHistory.some(row=>JSON.parse(row.checks_json).some(check=>check.field_key==='external_finish'&&check.expected_value==='Black')),'Original review evidence was lost during correction');
