...
test("the canonical entry point and unified server PDF path are present and misleading legacy outputs are not exposed", async () => {
  ...
  assert.match(collectionView, /<span>Email<\/span><span>Follow Up<\/span><span>Status<\/span><span>Copy<\/span><span>Remove<\/span><span>Open<\/span>/);
  ...
  assert.match(preview, /Email ready to review/);
  assert.match(preview, /Estimate sent successfully/);
  assert.match(preview, /The PDF and Email draft are still saved/);
  assert.match(preview, /View details/);
