...
const transport=createDisposableGoogleTransport({messages,attachments,files:[{id:'disposable-root',name:'Disposable Estimates',mimeType:'application/vnd.google-apps.folder',parents:[],trashed:false}],delivery:delivery?{allowedRecipients:['customer.journey@example.test',...(factoryDelivery?['factory.journey@example.test']:[])],failFirst:!['supplier-followup','supplier-reconcile'].includes(process.env.QUOTESUITE_DISPOSABLE_PROVIDER_DELIVERY),loseFactoryResponse:['factory-reconcile','supplier-reconcile'].includes(process.env.QUOTESUITE_DISPOSABLE_PROVIDER_DELIVERY)}:null});
...
globalThis.fetch=async(url,options)=>{
 ...
 if(delivery&&parsed.hostname==='gmail.googleapis.com'&&parsed.pathname.endsWith('/messages/send'))await writeFile(path.join(root,'disposable-delivery-evidence.json'),JSON.stringify(transport.deliveryEvidence));
 return response;
};
