...
test("Basic RAMS preserves issued revisions and detects later schedule changes",async t=>{
  ...
  assert.equal(record.issued.sourceSnapshot.qualificationReview.teamId,team.id);assert.equal(record.issued.sourceSnapshot.qualificationReview.attendanceEnd,'2026-09-18');
  const issuedWorkforce=JSON.stringify(record.issued.sourceSnapshot.qualificationReview);await db.run('UPDATE installation_installers SET active=0 WHERE id=?',installer.id);const changedTeam=await safety.get(record.id);assert.equal(changedTeam.status,'review_required');assert.equal(JSON.stringify(changedTeam.issued.sourceSnapshot.qualificationReview),issuedWorkforce,'Later staff changes must not rewrite the issue check');
  await assert.rejects(()=>safety.saveDraft(record.id,{requiredQualificationTypes:'cscs'}),/required qualification choices/);
