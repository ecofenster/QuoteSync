...
test('lost factory response is verified from bounded SENT search and exact MIME attachment bytes',async()=>{
 ...
 const attempt={receipt_message_id:'<quotesuite-factory-abcdef@delivery.quotesuite.invalid>',receipt_manifest_sha256:factoryManifest(saved)};
 ...
 await assert.rejects(()=>provider.send({...saved,attachments:[{...saved.attachments[0],bytes}],factoryReceipt:{messageId:attempt.receipt_message_id,manifestSha256:attempt.receipt_manifest_sha256}}),/response lost/);
 const raw=await provider.findFactoryReceipt(attempt.receipt_message_id),verify=(value=raw,retained=saved,readAttachment=provider.attachment)=>verifyFactoryReceipt({raw:value,attempt,saved:retained,readAttachment});
