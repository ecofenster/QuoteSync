...
test("chronology remains ordered and displays the current checkpoint", () => {
  assert.deepEqual(ROADMAP_CHRONOLOGY.map((entry) => entry.sequence), Array.from({ length: 42 }, (_, index) => index + 1));
  assert.equal(ROADMAP_CHRONOLOGY.find((entry) => entry.title === "Checkpoint stabilization")?.checkpointSha, ROADMAP_CHECKPOINT_SHA);
  assert.match(ROADMAP_CHRONOLOGY.at(-1)?.title ?? "", /Shared control and semantic-colour consolidation/);
