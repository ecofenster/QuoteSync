...
test("chronology remains ordered and displays the current checkpoint", () => {
  assert.deepEqual(ROADMAP_CHRONOLOGY.map((entry) => entry.sequence), Array.from({ length: 134 }, (_, index) => index + 1));
  ...
  assert.equal(ROADMAP_CHRONOLOGY.at(-1)?.title, "Mailbox and reading-pane mode boundary correction");
