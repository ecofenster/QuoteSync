...
test("chronology remains ordered and displays the current checkpoint", () => {
  assert.deepEqual(ROADMAP_CHRONOLOGY.map((entry) => entry.sequence), Array.from({ length: 147 }, (_, index) => index + 1));
  ...
  assert.equal(ROADMAP_CHRONOLOGY.at(-1)?.title, "Guided supplier RFQ preparation");
  assert.equal(ROADMAP_CHECKPOINT_SHA, "0c4ed0f");
});
