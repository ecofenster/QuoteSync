...
test("chronology remains ordered and displays the current checkpoint", () => {
  assert.deepEqual(ROADMAP_CHRONOLOGY.map((entry) => entry.sequence), Array.from({ length: 133 }, (_, index) => index + 1));
  ...
  assert.equal(ROADMAP_CHRONOLOGY.at(-1)?.title, "Individual-message mailbox projection correction");
