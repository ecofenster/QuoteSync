...
test("chronology remains ordered and displays the current checkpoint", () => {
  assert.deepEqual(ROADMAP_CHRONOLOGY.map((entry) => entry.sequence), Array.from({ length: 152 }, (_, index) => index + 1));
  ...
  assert.equal(ROADMAP_CHRONOLOGY.at(-1)?.title, "Controlled Portal, Order and supplier-confirmation journey");
  assert.equal(ROADMAP_CHECKPOINT_SHA, "3b3fd4d");
});
