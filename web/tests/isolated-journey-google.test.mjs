...
test('no-network reissue mode needs isolated development, explicit opt-in and exact test addresses',()=>{
 ...
 assert.equal(isolatedJourneyMode({...env,QUOTESUITE_DISPOSABLE_PROVIDER_DELIVERY:'factory-reconcile'}).factoryDelivery,true);
 assert.throws(()=>isolatedJourneyMode({...env,QUOTESUITE_DISPOSABLE_PROVIDER_DELIVERY:'factory-reconcile',NODE_ENV:'production'}),/requires the owned/);
