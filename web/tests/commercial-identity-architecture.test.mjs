...
test("reviewed existing-Client email filing creates Estimate → Suppliers → named supplier folders and reuses the genuine document", async (t) => {
  ...
  const preflight=await communications.reviewSupplierDocumentAssignment("disposable-message",selection);
  assert.equal(preflight.conflict.kind,"new_file");assert.equal(preflight.conflict.recommendedDecision,"save");assert.equal(preflight.destinationExists,false);
  const first=await communications.assignSupplierDocument("disposable-message",{...selection,fileDecision:preflight.conflict.recommendedDecision});
  const retainedReview=await communications.reviewSupplierDocumentAssignment("disposable-message",selection);
  assert.equal(retainedReview.conflict.kind,"already_filed");assert.equal(retainedReview.conflict.existingFile.id,"provider-document-1");
  const second=await communications.assignSupplierDocument("disposable-message",{...selection,fileDecision:retainedReview.conflict.recommendedDecision});
  ...
  const revisedInput={clientId:"client",projectId:project.id,estimateId:"estimate",supplierCode:"ZYLE",communicationAttachmentId:"different-message_attachment_part-1",providerMessageId:"different-message",providerAttachmentId:"different-provider-attachment",fileName:"EF-CL-028 Stuart Gilks, Ecotherm +.docx",mediaType:"application/vnd.openxmlformats-officedocument.wordprocessingml.document",bytes:Buffer.from("revised supplier evidence")};
  await assert.rejects(()=>drive.storeCommunicationSupplierDocument(revisedInput),error=>error.code==="communication_assignment_file_review_required"&&error.details.conflict.kind==="same_name_different_content");
  const revision=await drive.storeCommunicationSupplierDocument({...revisedInput,fileDecision:"save_new_revision"});
  assert.equal(revision.fileName,"EF-CL-028 Stuart Gilks, Ecotherm + (revision 2).docx");assert.equal(revision.decision,"save_new_revision");assert.equal(uploads,2);
