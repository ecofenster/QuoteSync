...
3. **Customer Estimate, quotation and release (23)** — reuses manufacturer evidence and the existing document renderer. Download, deterministic preparation, failure/retry, provider-confirmed issue, immutable release and three-day Follow Up now pass one disposable normal-route journey. Remaining implementation is the shared Terms/exclusions/validity snapshot and explicitly recorded state gaps; the corrected EF-EST-2026-055 visual/PDF and full issue journey still need user acceptance. Production sending remains a controlled external dependency.
...
- Package 3 has started with deterministic customer Estimate preparation and a verified Download → prepare → failure/retry → immutable issue/release → Follow Up browser journey. Continue at the shared Terms/exclusions/validity snapshot before the consolidated user checklist.
