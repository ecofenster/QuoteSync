...
2. **Email, Files and supplier-document intake (23)** — builds on CRM context. Individual-message reading, exact selected-attachment ownership, evidence-backed link-or-create intake, stable partial-failure retry, supplier-folder conflict classification, save-as-new-revision, progress/failure/retry feedback and import handoff now have disposable normal-route evidence. A working Estimate also opens guided supplier RFQ preparation with current supplier/file ownership, immutable attachment snapshots, revision lineage and recoverable preview-only Email draft creation. Remaining implementation is the joined returned-quote match/event/import route plus wider provider operations; the bounded Gmail/RFQ walkthrough still needs user acceptance. Genuine provider acceptance requires configured Gmail/Drive, but no live filing is authorised for EF-CL-028.
...
- Package 2 Email intake/filing and initial supplier-RFQ preparation are technically verified through likely-record link-or-create → working Estimate RFQ draft → selected message → destination check → explicit revision choice → progress/failure/retry → exact result and next actions. Continue at the normal-route returned-response match → Quote Returned event → Manufacturer Import review handoff; do not expand into non-Gmail channels or live EF-CL-028 writes.
