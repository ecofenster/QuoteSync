...
# Incremental change: updates src\App.tsx only.
#
# Run from: PS C:\Github\QuoteSync\web>
#
# What it does:
# - Verifies you're in C:\Github\QuoteSync\web
# - Backs up .\src\App.tsx to C:\Github\QuoteSync\_backups\quotesync_patch\
# - Overwrites .\src\App.tsx with the patched full file content
#
# Future-config hook:
# - Reads optional localStorage key: quotesync.estimateRefPrefix (default EF-EST)
...
$expected = "C:\Github\QuoteSync\web"
if ((Get-Location).Path -ne $expected) {
  Fail "Run this from: PS $expected>  (current: $((Get-Location).Path))"
}
...
Write-Host ""
Write-Host "Done. Restart dev server if needed: npm run dev" -ForegroundColor Cyan
